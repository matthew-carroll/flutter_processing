...
class Sketch {
  ...
  void noiseSeed(int? seed) {
    _initializePerlinNoise(seed: seed);
  }
  ...
  /// Sets the number of [octaves] and the [falloff] of each octave
  /// for values generated by [noise()].
  void noiseDetail({
    int? octaves,
    double? falloff,
  }) {
    _perlinNoiseOctaves = octaves ?? _perlinNoiseOctaves;
    _perlinNoiseFalloff = falloff ?? _perlinNoiseFalloff;
  ...
  double randomGaussian({
    ...
    double y1, x1, x2, w;
    if (_gaussianPrevious != null) {
      y1 = _gaussianPrevious!;
      _gaussianPrevious = null;
    } else {
      do {
        x1 = this.random(2) - 1;
        x2 = this.random(2) - 1;
  ...
  Future<void> save({
    ...
    if (format != null) {
      ...
      final imageFormatFromFile = _getImageFileFormatFromFilename(file.path);
  ...
  /// function is run, saved in the given [directory].
  ///
  ...
  Future<void> saveFrame({
    ...
    if (format != null) {
      ...
      final imageFormatFromFile = _getImageFileFormatFromFilename(namingPattern);
      if (imageFormatFromFile == null) {
        throw Exception(
            'Cannot save frame image to file with invalid extension and no explicit image type: $namingPattern');
      }
      imageFormat = imageFormatFromFile;
    }
    ...
    if (hashMatches.isEmpty) {
      ...
      throw Exception(
          'namingPattern has too many hash patterns in it. There must be exactly one series of hashes in the pattern.');
    }
  ...
  ImageFileFormat? _getImageFileFormatFromFilename(String filePath) {
