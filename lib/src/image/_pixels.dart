...
mixin SketchImagePixels on BaseSketch {
  Future<Color> get(int x, int y) async {
    return await _paintingContext.canvas.get(x, y);
    // await _paintingContext.canvas.doIntermediateRasterization();
  ...
  Future<Image> getRegion({
    ...
    return await _paintingContext.canvas.getRegion(x: x, y: y, width: width, height: height);
    //
  ...
  Future<void> loadPixels() async {
    await _paintingContext.canvas.startBitmapTransaction();
    // await _paintingContext.loadPixels();
  }

  ByteData? get pixels => _paintingContext.canvas.pixels;
  ...
  void set({
    ...
    _paintingContext.canvas.set(x: x, y: y, color: color);
  ...
  Future<void> setRegion({
    ...
    await _paintingContext.canvas.setRegion(image: image);
  ...
  Future<void> updatePixels() async {
    await _paintingContext.canvas.endBitmapTransaction();
